...
- (void)didMoveToWindow
{
  if (self.window != nil && _webView == nil) {
    ...
    //      [recognizer requireGestureRecognizerToFail:gr];
    //    }
    //  }
    //}
}
...
- (void)removeGestureRecognizers
{
  for (UIView* view in _webView.subviews) {
      for (UIView* subview in view.subviews) {
        NSLog(@"***SUBVIEW ON REMOVE %@ WITH GESTURES %@", subview, subview.gestureRecognizers);
        for (UIGestureRecognizer* recognizer in subview.gestureRecognizers) {
          NSLog(@"***RECOGNIZER %@", recognizer);
          NSString *className = @"UILongPressGestureRecognizer";
          NSString *secondClassName = @"_UITextSelectionForceGesture";
          /*
          if ([NSStringFromClass([recognizer class]) isEqual:className]) {
            NSLog(@"***REMOVE");
            [subview removeGestureRecognizer:recognizer];
          }
          if ([NSStringFromClass([recognizer class]) isEqual:secondClassName]) {
            NSLog(@"***SECOND REMOVE");
            [subview removeGestureRecognizer:recognizer];
          }
          */
